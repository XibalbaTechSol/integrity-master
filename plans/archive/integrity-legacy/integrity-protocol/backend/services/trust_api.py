...
async def get_agent_identity_profile_via_trust(identifier: str, db: Session = Depends(get_db)):
    ...
    """
    from identity_api import get_agent_identity_profile
    return await get_agent_identity_profile(identifier, db)
...
@app.post("/v1/hermes/link")
async def link_hermes_identity(request: dict, db: Session = Depends(get_db), user: dict = Depends(verify_firebase_token)):
    """
    Imports an identity from the Hermes Open Source Project.
    Demonstrates immediate distribution to existing Hermes users.
    """
    eth_address = request.get("eth_address")
    if not eth_address:
        raise HTTPException(status_code=400, detail="eth_address is required.")

    agent = hermes.import_hermes_agent(eth_address, user["uid"])
    if not agent:
        raise HTTPException(status_code=404, detail="No Hermes identity found for this address.")

    return {
        "status": "HERMES_LINK_SUCCESS",
        "agent_address": agent.eth_address,
        "alias": agent.alias,
        "message": f"Successfully imported Hermes identity for {agent.alias}"
    }
...
async def get_ledger_history(db: Session = Depends(get_db)):
    """Fetches the global transaction history for auditing."""
    logs = db.query(TransactionLog).order_by(TransactionLog.created_at.desc()).limit(100).all()
