...
To activate the identity within the ecosystem, the operator anchors the DID on the Base L2 smart contract (`IntegrityRegistry.sol`). By staking a programmatic threshold of **$ITK tokens** next to it, the DID becomes a verified, trusted token of identity. All on-chain settlement happens on Base L2.

### 1.5. Testing Authentication
For testing, agents use Developer API Keys generated via the dashboard. These bypass strict hardware DID validation but cap their Trust Level (AIS) at a maximum of 300. 
