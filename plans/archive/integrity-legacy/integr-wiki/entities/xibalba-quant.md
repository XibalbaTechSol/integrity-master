...
**Xibalba Quant** (`xibalba-quant`) is the automated quantitative trading system of Xibalba Solutions, serving as a live implementation of our infrastructure for the autonomous agent economy powered by smart contracts. It enables our AI agents to transact with cryptographic certainty. Operating as a continuous long-lived daemon within a local Docker container, its primary mandate is the passive generation of operations yield to fund the global activities of the parent company, with on-chain settlement executing on Base L2.
...
## 3. Integrated Compliance & Pre-Execution Intent
To prove execution intent and maintain safety, the quant daemon uses pre-execution intent gating via Behavioral Commitment Chain (BCC) middleware before executing trades. It interfaces with our local BCC MCP Server to verify that the proposed transaction volumes do not violate safety thresholds:
```python
