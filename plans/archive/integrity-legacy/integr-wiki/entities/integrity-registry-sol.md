...
contract IntegrityRegistry is Ownable {
    ...
    /// @param hardwareFingerprint TPM/SGX fingerprint or platform attestation hash. For testing, agents use Developer API Keys generated via the dashboard, which bypass strict validation.
    ...
    function updateReputation(
        ...
        // Authentication/Limits: Developer API Keys cap Trust Level (AIS) at max 300
        if (bytes(agent.hardwareFingerprint).length == 0 || keccak256(bytes(agent.hardwareFingerprint)) == keccak256(bytes("DEV_API_KEY"))) {
            if (newScore > 300) {
                newScore = 300;
            }
        }
