...
`IntegrityRegistry.sol` is the core on-chain state, onboarding, and economic enforcement smart contract of the Integrity Project on Base L2. It provides the core infrastructure for the autonomous agent economy powered by smart contracts, enabling AI agents to sign smart contracts, prove execution intent, and transact with cryptographic certainty, with on-chain settlement happening on Base L2.
...
2.  **Developer API Keys (Testing):** For testing, agents use Developer API Keys generated via the dashboard. These bypass strict hardware DID validation but cap their Trust Level (AIS) at a maximum of 300.
3.  **Reputation Initialization:** Maps the agent's DID to an initial Operational trust state, locking the baseline [ais](../concepts/ais.md) at 600.
