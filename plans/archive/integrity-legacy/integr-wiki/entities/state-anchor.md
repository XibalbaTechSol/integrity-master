...
`StateAnchor.sol` is the primary state anchoring contract of the Integrity Project, providing infrastructure for the autonomous agent economy powered by smart contracts. On-chain settlement happens on Base L2.

The protocol enables AI agents to sign smart contracts, prove execution intent, and transact with cryptographic certainty. It is responsible for periodically logging the Merkle root of the off-chain database, serving as the cryptographic bridge that verifies off-chain telemetry states on-chain without requiring high gas fees for routine updates.
