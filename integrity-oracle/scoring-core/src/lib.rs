...
impl Default for TriMetricScoringEngine {
    fn default() -> Self {
        ...
        // To minimize Euclidean distance to the legacy targets under the constraint that the weights sum to 1.0,
        // we use Lagrange multipliers (orthogonal projection).
        // The optimal weights are derived by shifting each coordinate by exactly +1/15.
