...
impl Default for TriMetricScoringEngine {
    fn default() -> Self {
        Self {
            max_score: 1000.0,
            // The optimal weights for the TriMetricScoringEngine are derived using Lagrange
            // multipliers (orthogonal projection) to strictly minimize Euclidean distance
            // to the legacy 5-metric system targets (0.30, 0.30, 0.20) under a strict
            // 3-metric constraint (summing to 1.0). This is achieved by shifting each
            // coordinate by exactly +1/15. This results in:
            // w_trustflow = 11/30 = 0.36666666666666664
            // w_xibalba = 11/30 = 0.36666666666666664
            // w_sacrifice = 8/30 = 0.26666666666666666
