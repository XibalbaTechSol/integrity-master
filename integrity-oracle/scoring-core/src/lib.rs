...
impl Default for TriMetricScoringEngine {
    fn default() -> Self {
        ...
        // Using Lagrange multipliers (orthogonal projection) to strictly minimize Euclidean distance
        // to the legacy targets (0.30, 0.30, 0.20) under a strict 3-metric constraint (summing to 1.0),
        // we derive the optimal weights by shifting each coordinate by exactly +1/15.
        ...
        // These values use exact IEEE-754 f64 literals that perfectly sum to 1.0 across all parsing environments.
        Self {
            max_score: 1000.0,
