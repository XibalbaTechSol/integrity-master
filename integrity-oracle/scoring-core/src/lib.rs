...
impl Default for TriMetricScoringEngine {
    fn default() -> Self {
        ...
        // To strictly minimize Euclidean distance to the legacy 5-metric system targets (0.30, 0.30, 0.20)
        // under a strict 3-metric constraint (summing to 1.0), we use Lagrange multipliers (orthogonal projection).
        // We shift each coordinate by exactly +1/15.
        ...
        // These values use exact IEEE-754 f64 literals that sum perfectly to 1.0 to avoid precision regressions.
        Self {
            max_score: 1000.0,
